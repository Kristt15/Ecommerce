...
$stmt->execute();
$qPesanan = $stmt->get_result();

// Status counts for tab badges — no user input, plain query is fine
$statusCounts = [];
...
<form method="GET" class="mb-3 d-flex gap-2" style="max-width:380px;">
    ...
    <?php if ($search): ?>
        <a href="pesanan.php<?= $statusFilter ? '?status=' . $statusFilter : '' ?>"
        ...
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        <td>
                            <span class="fw-semibold text-primary"># <?= $row['id'] ?></span>
                        </td>
                        <td>
                        ...
                        <td class="fw-semibold">Rp <?= number_format($row['total'], 0, ',', '.') ?></td>
                        <td>
                        ...
                        <td>
                            <?php if ($row['status'] !== 'selesai' && $row['status'] !== 'batal'): ?>
                                <form action="../proses/proses_update_status_pesanan.php" method="POST"
                                    class="d-flex gap-1">
                        ...
                        <td colspan="7" class="p-0">
                            <div class="px-3 py-2">
                                ...
                                $qDetail = $conn->prepare("
                                    ...
                                    WHERE dp.pesanan_id = ?
                                ");
                                $qDetail->bind_param('i', $row['id']);
                                $qDetail->execute();
                                $rDetail = $qDetail->get_result();
                                ?>
                                <div class="d-flex flex-wrap gap-2">
                                    <?php while ($d = mysqli_fetch_assoc($rDetail)): ?>
                                        <span class="badge bg-light text-dark border"
                                            style="font-weight:400;font-size:.8rem;">
                ...
                <?php if ($empty): ?>
                    <tr>
                        <td colspan="7" class="text-center py-5 text-muted">
                            ...
                            <?= $statusFilter
                                ? 'Tidak ada pesanan dengan status "' . htmlspecialchars($statusFilter) . '".'
                                : 'Belum ada pesanan.' ?>
                        </td>
                    </tr>
                <?php endif; ?>
